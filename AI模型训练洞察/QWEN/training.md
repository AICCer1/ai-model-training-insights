...
flowchart LR
    ...
    subgraph M[Qwen 3.5 397B A17B MoE]
        ...
        M4[MoE<br/>512 专家<br/>激活 10 路由 + 1 共享]
        M5[重复 15 组]
        M6[60 层<br/>262K 原生上下文]
        M1 --> M2 --> M3 --> M4 --> M5 --> M6
    end
...
### 阿里云 PAI / 灵骏
围绕 Qwen 系列，公开资料里最值得看的不是只有模型本身，还有：
- **PAI 灵骏智算服务**
- **Qwen 在 PAI 上的训练 / 精调 / 部署最佳实践**

从公开文档里可以看到：
- 面向大规模分布式 AI 研发场景
- 支持 RDMA、K8s 兼容、主流训练框架
- 支持任务界面化提交和日志查看
- 明确面向通用大模型训练和推理业务

### 对这个仓库的意义
这说明 Qwen 这条线除了训练细节，也很适合观察：
- 模型与云上智算平台如何协同
- 训练平台怎样把底模、微调、部署串起来
